...
# cmake . -DCMAKE_BUILD_TYPE=Release -DFRUIT_USES_BOOST=False -DBUILD_SHARED_LIBS=False -DCMAKE_SYSTEM_NAME=Android -DCMAKE_ANDROID_API=24 -DANDROID_PLATFORM=24 -DCMAKE_ANDROID_ARCH_ABI=arm64-v8a -DCMAKE_ANDROID_NDK=$ANDROID_NDK_ROOT -DCMAKE_ANDROID_STL_TYPE=c++_static -DCXX_STANDARD=20 -DCMAKE_TOOLCHAIN_FILE=$ANDROID_NDK_ROOT/build/cmake/android.toolchain.cmake -DANDROID_TOOLCHAIN=clang -DFRUIT_TESTS_USE_PRECOMPILED_HEADERS=OFF && make install DESTDIR=../ -j
cmake . -DCMAKE_BUILD_TYPE=Release -DFRUIT_USES_BOOST=False -DBUILD_SHARED_LIBS=False -DCMAKE_SYSTEM_NAME=Android -DCMAKE_ANDROID_API=24 -DANDROID_PLATFORM=24 -DCMAKE_ANDROID_ARCH_ABI=arm64-v8a -DCMAKE_ANDROID_NDK=$ANDROID_NDK_ROOT -DCMAKE_ANDROID_STL_TYPE=c++_static -DCMAKE_CXX_STANDARD=20 -DCMAKE_TOOLCHAIN_FILE=$ANDROID_NDK_ROOT/build/cmake/android.toolchain.cmake -DFRUIT_TESTS_USE_PRECOMPILED_HEADERS=OFF && make install DESTDIR=../ -j
